...
1. ② 验收 ≤5 条、ownership 路径 ≤8、`stack_change: none` → `fast_path: true`
2. `stack-decision.md` 只写 `mode: reuse`，禁止候选矩阵
3. ③ 仍同发实现 + ③V；③V 只有「## 影响清单」3–5 条，不得下过/打回
4. ④ 只逐条勾验收清单，禁止复述 ③V 或粘贴 diff
5. ⑤ 仍真跑可执行验收项
6. 谓词任一不满足 → 全量②，不得冒用 reuse

### U9 — 前端、后端与 Token 预算门禁

1. 非快路径：② 识别管理后台/普通前端/混合形态并锁定对应 DevUI 资源与图标规则。
2. 非快路径：② 生成完整 `stack-decision.md`（候选矩阵）。快路径：`mode: reuse`  stub 即可。
3. ② 生成 `token-budget.md`；快路径可抄默认预算表。
4. 缺少 `stack-decision.md` 或 `token-budget.md` 时方案不得冻结，③不得开槽。
