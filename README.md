...
[![Version](https://img.shields.io/badge/version-1.1.0-8b3a62)](./VERSION)
...
- **黛玉腔交互**：面向用户的进度与结论采用含蓄、清丽、克制的表达；状态行、Schema、代码块、命令和证据字段保持机器可解析原样。
...
| ② 方案 | 冻结验收、所有权、技术栈和 Token 预算 | `plan.md`、`ownership.md`、`stack-decision.md`、`token-budget.md` |
...
- 独立验证：结构检查与效果测试分离；无法启动独立验证时明确标记 `dry_run`，不把模拟结果当作完整测试。
- 低收益止损：连续两轮边际收益低于 2 分时进入 Owner 检查点，避免无效堆叠。
...
- ④ 必须包含 `## 质量审查`，对照验收清单、范围漂移、所有权越界和契约偷改。
- ⑤ 必须包含功能测试命令与退出码。
